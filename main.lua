...
TITLE.Text = "| NIKZITTXE'S AUTOFARM - @ewwcringe"
...
local Log = function(Msg)
	print(os.date("%X").." | nikzittxe's autofarm - "..Msg)
end
		...
		task.spawn(function()
			if (os.time() >= LastLog) then 
				local s, e = pcall(function()
					SendLog(_G.AutofarmSettings.Webhook, {Player.Name, Player.UserId, WALLET.Text, PROFIT.Text, TIMER.Text, BROKEN.Text, "| nikzittxe's autofarm by @ewwcringe"})
				end)
				if (e) then 
